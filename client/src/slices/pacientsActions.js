...
export const getPacient = createAsyncThunk('pacients/getPacient', async (id) => {
    try {
        const response = await axios.get(`http://192.168.0.14/api/users/${id}`)
...
export const postPacient = createAsyncThunk('pacients/postPacient', async (newPacient) => {
    try {
        ...
        const response = await axios({
            method: "post",
            url: "http://192.168.0.14/api/users",
            data: newPacient,
        });
        return response.data.data
    } catch (error) {
...
export const putPacient = createAsyncThunk('pacients/putPacient', async ({_id, ...pacient}) => {
    try {
        const response = await axios.put(`http://192.168.0.14/api/users/${_id}`, pacient)      //NO SE PORQUÉ SI PONGO AWAIT NO ANDA (EN PROF, ACÁ NO SE)
        return response.data.data
    } catch (error) {
        return error.message
    }        
...
export const deletePacient = createAsyncThunk('pacients/deletePacient', async (id) => {
    try {
        const response = await axios.delete(`http://192.168.0.14/api/users/${id}`)      //NO SE PORQUÉ SI PONGO AWAIT NO ANDA (EN PROF, ACÁ NO SE)
        return response.data.data
    } catch (error) {
        return error.message
    }        
